...
class PyBadgeHardware(BaseHardware):
    ...
    def poll(self) -> InputState:
        b = self._hw.button
        current = {name: bool(getattr(b, attr)) for name, attr in self._BUTTON_ATTRS.items()}

        state = InputState()
        state.buttons         = current
        state.buttons_pressed = {k for k, v in current.items() if v and not self._prev[k]}
        state.buttons_released = {k for k, v in self._prev.items() if v and not current[k]}
